// Add Transaction Bottom Sheet — Shared shells, primitives, helpers
// Split from v4-add-transaction-e-improved.jsx + v4-add-transaction-d-numpad.jsx
// Loaded once before transaction_1.jsx ... transaction_8.jsx
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Scrollable content */}
        <div data-flip-source="form-content" style={{ flex: 1, overflow: 'auto' }}>
          {children}
        </div>
...
function EAmountHero({ amount = '85', sign = '−' }) {
  return (
    <div data-flip-source="amount-hero" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div style={{ fontSize: 11, color: TE.n400, fontWeight: 600, letterSpacing: 0.4, textTransform: 'uppercase' }}>จำนวนเงิน</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{
            fontSize: 11, fontWeight: 700, color: TE.n600,
            background: TE.n200, padding: '3px 8px', borderRadius: 8
          }}>THB ▾</div>
          <div style={{
            width: 30, height: 30, borderRadius: 8, background: TE.n200,
            display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M4 7V5a1 1 0 011-1h2M20 7V5a1 1 0 00-1-1h-2M4 17v2a1 1 0 001 1h2M20 17v2a1 1 0 01-1 1h-2"
              stroke={TE.n600} strokeWidth="1.8" strokeLinecap="round" />
              <rect x="8" y="9" width="8" height="6" rx="1" stroke={TE.n600} strokeWidth="1.8" />
            </svg>
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
        <div style={{ fontSize: 44, fontWeight: 800, color: TE.n900, letterSpacing: -1, fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
          {sign}{Number(amount.replace(/,/g,'')).toLocaleString('en-US')}
          <span style={{ color: TE.n400, fontWeight: 500 }}>.00</span>
        </div>
        <div style={{
          width: 2, height: 30, background: TE.primary500,
  ...
  return (
    <div style={{ padding: '0 16px 12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
            <rect x="3" y="4" width="18" height="18" rx="2" stroke={TE.primary500} strokeWidth="1.5"/>
            <path d="M3 10h18" stroke={TE.primary500} strokeWidth="1.5"/>
            <circle cx="6" cy="14" r="1" fill={TE.primary500}/>
            <circle cx="12" cy="14" r="1" fill={TE.primary500}/>
            <circle cx="18" cy="14" r="1" fill={TE.primary500}/>
            <circle cx="6" cy="19" r="1" fill={TE.primary500}/>
            <circle cx="12" cy="19" r="1" fill={TE.primary500}/>
            <circle cx="18" cy="19" r="1" fill={TE.primary500}/>
          </svg>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
            <path d="M4 4h16v14H4z" stroke={TE.primary500} strokeWidth="1.5"/>
            <path d="M4 8h16M4 12h10M4 16h8" stroke={TE.primary500} strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
              <path d="M20.59 13.41l-7.17 7.17a2 2 0 01-2.83 0L2 12V2h10l8.59 8.59a2 2 0 010 2.82z" 
                    stroke={TE.primary500} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
              <circle cx="6.5" cy="6.5" r="1.5" fill={TE.primary500}/>
            </svg>
...
// ─── Wallet + Category 2-column tile row ──────────────────
function EWalletCatRow({ catValue, onCatChange, walletLabel = 'ครอบครัว', walletHint = '34,368฿' }) {
  const [pickerOpen, setPickerOpen] = React.useState(false);
  const [pickerCat, setPickerCat] = React.useState(null);

  const selectedCat = catValue ? findCat(catValue.key) : null;
  const selectedSub = selectedCat && catValue?.subKey ? findSub(selectedCat, catValue.subKey) : null;
  ...
  const pickCategory = (cat) => {
    if (!cat.subs || cat.subs.length === 0) {
      onCatChange?.({ key: cat.key, subKey: null });
      closePicker();
    } else {
      setPickerCat(cat);
    }
  ...
  const pickSub = (cat, sub) => {
    onCatChange?.({ key: cat.key, subKey: sub.key });
    closePicker();
  };
  const pickCategoryOnly = (cat) => {
    onCatChange?.({ key: cat.key, subKey: null });
  ...
  return (
    <div style={{ padding: '0 16px 10px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      ...
      <div style={tileBase}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, color: TE.n400, fontWeight: 600, letterSpacing: 0.2 }}>กระเป๋า</div>
          <div style={{
            fontSize: 14, fontWeight: 700, color: TE.n900, letterSpacing: 0.1,
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
          }}>{walletLabel}</div>
          <div style={{ fontSize: 10, color: TE.n400, fontWeight: 500 }}>{walletHint}</div>
        </div>
      ...
      {/* Category tile */}
      <div onClick={openPicker} style={tileBase}>
        <div style={{
          width: 36, height: 36, borderRadius: 12,
          background: selectedCat ? selectedCat.bg : TE.n200,
          display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
        }}>
          {selectedCat ? (
            <CatIcon kind={selectedCat.icon} containerSize={36} color={selectedCat.ic} />
          ) : (
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
              <circle cx="5" cy="6" r="1.4" fill={TE.n500}/>
              <circle cx="5" cy="12" r="1.4" fill={TE.n500}/>
              <circle cx="5" cy="18" r="1.4" fill={TE.n500}/>
              <path d="M10 6h10M10 12h10M10 18h10" stroke={TE.n500} strokeWidth="2" strokeLinecap="round"/>
            </svg>
          )}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, color: TE.n400, fontWeight: 600, letterSpacing: 0.2 }}>หมวดหมู่</div>
          {selectedCat ? (
            <>
              <div style={{
                fontSize: 14, fontWeight: 700, color: TE.n900, letterSpacing: 0.1,
                whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
              }}>{selectedCat.label}</div>
              {selectedSub && (
                <div style={{
                  fontSize: 10, color: TE.n400, fontWeight: 500,
                  whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                }}>{selectedSub.label}</div>
              )}
            </>
          ) : (
            <div style={{ fontSize: 14, fontWeight: 600, color: TE.n400 }}>เลือก…</div>
          )}
        </div>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
          <path d="M6 9l6 6 6-6" stroke={TE.n400} strokeWidth="2" strokeLinecap="round"/>
...
// ─── D-style helpers (CATEGORY_COLORS, CatChipIcon, DSheetShell, DTabs, etc) ───
...
// ─── Amount Card (Variation A style — with cursor blink) ────
function DAmountCard({ amount = '0', sign = '-', hint, selectedCat, quickChips = false }) {
  const cat = selectedCat || CATEGORY_COLORS[0];
  return (
    <div style={{
      margin: '0 16px 10px', background: '#fff', borderRadius: 18, padding: '20px 18px 18px',
      boxShadow: '0 1px 2px rgba(0,0,0,0.03), 0 4px 14px rgba(0,0,0,0.04)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
...
// Re-export everything shared globally for transaction_*.jsx files
Object.assign(window, {
  // E-style shell
  ESheetShell, ETabs, EAmountHero, EFieldRow,
  ECategoryChips, ECategoryPicker, EWalletCatRow,
  EMetaRow, ETagsRow, EReportToggle,
  // D-style helpers (still used by transaction sheets)
  CATEGORY_COLORS, WALLET_COLORS, CatChipIcon,
  DesignPanel, AddedTxnChip, AddedTxnList,
  DSheetShell, DTabs, DAmountCard, DAccountRow,
  DArrowConnector, DQuickChips, DMetaRow, DWalletIcon,
});
